...
app.post("/make-server-97c553b8/admin/upload", async (c) => {
  try {
    ...
    if (!mindeeApiKey) {
      ...
      console.log(`Using mock OCR data for invoice ${invoiceId} (MINDEE_API_KEY not configured)`);
...
app.post("/make-server-97c553b8/invoices/upload", async (c) => {
  try {
    ...
    if (!uploadToken) {
      ...
      if (!user) {
        return c.json({ error: 'Unauthorized - no token or auth provided' }, 401);
      }
      uploadedBy = user.id;
    } else {
...
async function processInvoiceOCR(invoiceId: string, storagePath: string) {
  try {
    ...
    if (!signedUrlData?.signedUrl) {
      throw new Error('Failed to create signed URL for OCR processing');
    }
    ...
    if (!mindeeApiKey) {
      console.log(`OCR processing skipped for invoice ${invoiceId}: MINDEE_API_KEY not configured`);
    ...
    formData.append('document', new Blob([fileBuffer]), 'invoice.pdf');
    ...
    if (!response.ok) {
      ...
      console.log(`Mindee API error for invoice ${invoiceId}: ${response.status} - ${errorText}`);
    ...
    if (!prediction) {
      throw new Error('No prediction data from Mindee API');
    }

    // Extract data
    const extractedData = {
      date: prediction.date?.value || null,
      supplier: prediction.supplier_name?.value || prediction.supplier?.value || null,
      totalAmount: prediction.total_amount?.value || prediction.total_incl?.value || null,
      vatAmount: prediction.total_tax?.value || prediction.total_vat?.value || null,
      hasVat: (prediction.total_tax?.value || prediction.total_vat?.value) ? true : false,
    ...
    console.log(`OCR processing error for invoice ${invoiceId}: ${error.message}`);
